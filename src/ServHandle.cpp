...
void	ServHandle::servStart(void) {
	...
	// 	n = epoll_wait(this->_epoll_fd, this->_event_ret, MAXEVENTS, -1);
	// 	std::cout << std::endl << MAG << "[INFO] " << n << " events" << reset << std::endl;
	...
	// 		}
	// 		else {
	// 			this->_tmpInt = this->findVec(this->_servFd, this->_event_ret[i].data.fd);
	// 			// std::cout << MAG << "[INFO]Server Socket Fd " << this->_servFd[this->_tmpInt] << " get access" << reset << std::endl;

	// 			if (this->_tmpInt >= 0) {

	// 				std::cout << CYN << "[INFO]show FLAG " << this->_event_ret[i].events << reset << std::endl;

	// 				// this->_inLen = sizeof(this->_inAddr);
	// 				// this->_infd = accept(this->_servFd[this->_tmpInt], &(this->_inAddr), &(this->_inLen));
	// 				this->_infd = accept(this->_servFd[this->_tmpInt], NULL, NULL);
	// 				if (this->_infd == -1) {
	...
	// 				// this->_bufferSend = generateHttpResponse(200, "OK", "Hello World");
	// 				// // send(int fd, const void *buf, size_t n, int flags)
	// 				// send(this->_infd, this->_bufferSend.c_str(), this->_bufferSend.size(), 0);
